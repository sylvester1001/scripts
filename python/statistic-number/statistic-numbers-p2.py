...
def analyze_continuous_numbers(input_filename, output_filename):
    ...
    # Modify the code to read and store numbers with two decimal places without rounding
    # numbers = sorted(set(df[0].apply(lambda x: float(f"{x:.2f}"))))
    numbers = sorted(set(df[0].apply(lambda x: float(x * 100) / 100)))
    # print(numbers)
    group = []
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        ...
        print(
            "Please enter both input file and output file as a command-line argument."
        )
        print("e.g. python statitic-number.py <input file name> <output file name>")
